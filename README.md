...
- **Per-role sprint field, written back into the issue** — a team of several roles in one project can keep its sprints in different issue fields; the sprint form shows one picker while the field is shared and a row per role once they differ. An opt-in switch (off by default) writes the chosen sprint value into every active issue of a confirmed role composition, under the user's own permissions, with a per-issue report. The value is only ever set, never cleared.
- **Excluded issues with a reason** — switching an issue to “Excluded from sprint” asks for a reason (required, up to 500 characters); with cross-role exclusion on, the same reason and who/when stamp go to every role at once. Excluded issues leave the role table for an “Excluded from sprint (N)” block below it — reason, stamp, “Return” and “Edit reason”; the reason shows as a tooltip on the status in sprint history and as a column in the Excel export.
- **Work phases inside the sprint** — a “Work phases” block in Sprint parameters: six phases Analysis → Development → Tech test → Regression → Business test → Deploy with date pairs and a bar on the sprint scale, role chips from the phase × roles mapping in settings, order and out-of-range warnings, range refusals; phases are copied into every role snapshot and the Excel header.
...
- **Filters on the task tables** — an Assignee · State · Role · Priority row above the allocation tables and the assignee task table: multi-select, a «Tasks shown» counter, a «filter: N of M» chip on every role row, roles left out by the Role filter hidden entirely. The selection is shared by both screens and lives until the page is reloaded.
...
- **Gantt timeline per role or across all roles** — one track per sprint role, the task chain between roles, date conflicts with outlined bars and a counter, work phase bands; dates and assignees of any role are edited right on the chart. “Forecast dates” schedules all roles at once in dependency order and marks what stays without dates; subtasks are grouped under their parent epic.
