...
- **Set Field** writes one field per call with the token user's permissions; the node takes the field kind from the issue itself. Multi-value fields are not written in 0.1.0 — use a command of the YouTrack node. **Set Field via Planner** writes only the fields from the planner settings and only with a planner role (a restricted contract operation).
