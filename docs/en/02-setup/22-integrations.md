...
The contract is the file [`Integrations/openapi-sprint.yaml`](../../../Integrations/openapi-sprint.yaml) in the repository (OpenAPI 3.1): 46 operations in nine sections — sprint, history, capacity, calendar and absences, releases, reminders, sprint lock, administration, access. Any OpenAPI viewer opens it (Swagger UI, Redoc); the descriptions inside the file are in Russian. The contract version equals the planner version and moves with every release.
...
- The same rules apply to absences (`upsertAbsence`, `removeAbsence`), capacity (`upsertPerson`) and releases (`upsertRelease`, `setReleaseStatus`, `addReleaseIssues`, `removeReleaseIssues`, `removeRelease` — deleting one release since 3.51.0).

## Planner projects and your roles

Two requests a scenario needs before writing (since 3.51.0):

- **`POST filter-planner-projects`** — the main address only and without `projectKey`. The body `{"keys": ["DEMO", "SP"]}` carries project keys taken from the standard YouTrack REST (`GET /api/admin/projects?fields=shortName,name`), up to 5000 per request. The answer lists those where the planner is attached and the token’s user may read: `{"projects": [{"key", "name", "hasMirror", "disabled"?}]}`. The request is live: the server checks the keys on every call, so the project list is not stored but asked for before work.
- **`GET my-roles`** — the token user’s permissions in the project: `{"configured", "disabled", "instanceAdmin", "roles": {"editor", "assigner", "validator", "historyManager", "settingsManager", "planningManager", "releaseManager", "releaseEngineer", "sprintLockManager"}}`, true — the permission is held. A YouTrack administrator gets all of them except history clearing. If the needed permission is missing, stop and report instead of learning it from a write refusal.
...
## MCP tools inside the planner

From 3.50.0 the planner itself ships MCP tools (22 since 3.51.0) that YouTrack’s built-in MCP server runs (YouTrack 2025.3 and later): an AI agent in an IDE or chat connects to `{your YouTrack}/mcp?customToolPackages=smart-sprint-planner` with the user’s personal permanent token — nothing else to deploy. Ten tools read (project overview, working sprint, history, capacity, calendar, absences, releases, reminders, projects with the planner attached, your own permissions) and twelve make point writes (draft upload with a guard against overwriting an occupied slot, issues, sprint header, assignments, absences, capacity participants, releases, release deletion). Each tool calls the same handler as the address by project key above, so the permissions are exactly those of the token’s user and the planner roles of this chapter; the restricted operations are not exposed. A refusal arrives as a tool error with `reason` and `cid`. Write to one project sequentially: strictly simultaneous writes may lose changes (a YouTrack property), so an agent re-reads after a series of writes. Importing the app with the tools takes 13–33 s and briefly needs about 1 GB of free YouTrack heap — update in a quiet window.

## MCP server for AI agents

For AI agents (Claude Code, Kilo Code, Cursor and other MCP clients) the repository ships a ready-made [MCP server](../../../Integrations/mcp-server/README.md) on top of this contract: twenty-two tools — ten for reading and twelve point writes (draft upload with a guard against overwriting an occupied slot, issues, sprint header, assignments, absences, capacity participants, releases, release deletion) — plus reference resources and scenario prompts. It is a shared HTTP server in a container; every request carries the caller's personal YouTrack token in the `Authorization` header, so the permissions are exactly those of the person behind the agent. The restricted operations of this chapter are not exposed by the server at all.

## n8n node

For [n8n](https://n8n.io) there is a ready-made [community node](../../../Integrations/n8n-node/README.md) on top of this contract, npm package `n8n-nodes-smart-sprint-planner`: 26 operations over eleven resources — project overview and planner status, the working sprint (get, draft upload with a guard against overwriting an occupied slot, header), role issues, history, capacity, calendar, absences, releases (including deletion), reminders, your own roles, plus YouTrack issue search and field writes. The credential holds the YouTrack URL and the permanent token of the user whose permissions the node gets; the planner roles are those of this chapter. The node reads data revisions itself, carries them between the items of one run and retries a point write once on a conflict; a refusal arrives as a node error with `reason`, a hint and `cid`, or, with Continue On Fail, as an item `{ ok: false, … }`. Items are processed one after another; as with every client, write to one project sequentially and re-read after a series of writes. The node can also serve as an n8n AI Agent tool.
