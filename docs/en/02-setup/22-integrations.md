...
From 3.50.0 the planner itself ships 19 MCP tools that YouTrack’s built-in MCP server runs (YouTrack 2025.3 and later): an AI agent in an IDE or chat connects to `{your YouTrack}/mcp?customToolPackages=smart-sprint-planner` with the user’s personal permanent token — nothing else to deploy. Eight tools read (project overview, working sprint, history, capacity, calendar, absences, releases, reminders) and eleven make point writes (draft upload with a guard against overwriting an occupied slot, issues, sprint header, assignments, absences, capacity participants, releases). Each tool calls the same handler as the address by project key above, so the permissions are exactly those of the token’s user and the planner roles of this chapter; the restricted operations are not exposed. A refusal arrives as a tool error with `reason` and `cid`. Write to one project sequentially: strictly simultaneous writes may lose changes (a YouTrack property), so an agent re-reads after a series of writes. Importing the app with the tools takes 13–33 s and briefly needs about 1 GB of free YouTrack heap — update in a quiet window.
