...
Кнопка-иконка **«Скопировать ссылку»** стоит в шапке карточки планируемого релиза, рядом с экспортом, и в шапке спойлера релиза в «Истории релизов». Она есть у всех, кто видит релизы, в том числе без права править; у записей в спойлере **«Архив»** её нет.

Получатель попадает в «Планируемые релизы» или в «Историю релизов»:

- карточка прокручивается в центр и коротко подсвечивается, спойлер истории раскрывается;
- если планируемый релиз к этому моменту уже выпущен или отменён, откроются «Планируемые релизы» без подсветки — ищите его в «Истории релизов»;
- пункт **«Поделиться»** в рельсе с экранов релизов ведёт на экран целиком.

Кнопка видна там же, где «Поделиться»: только во входе через главное меню, в YouTrack 2026.1 и новее; на более старых версиях она скрыта и появится сама после обновления YouTrack ([глава 01](01-entry.md)).
