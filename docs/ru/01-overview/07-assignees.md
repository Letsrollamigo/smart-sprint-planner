...
**«+ Подобрать исполнителей»** открывает выбор людей, **«Очистить»** снимает всех, **«Обновить данные задач из YouTrack»** перечитывает данные задач.
...
Над таблицей — та же строка фильтров, что на экране [«Аллокация общего ресурса»](05-allocation.md), только без поля «Роль»: роль уже выбрана пикером вверху. Выбор общий для обоих экранов; таблица ресурсов исполнителей фильтром не затрагивается. Щелчок по заголовку «Состояние» сортирует задачи в порядке состояний YouTrack.
