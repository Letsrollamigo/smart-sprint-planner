...
Родителя-эпика отчёт берёт по роли **«Иерархия»** из экрана связей задач (глава 16 документа по внедрению) — той же, что строит дерево бэклога и состав релиза. Если строк в той таблице нет, работает прежняя эвристика по типу «Подзадача».
