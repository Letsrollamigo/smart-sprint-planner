 ...
 * в форк-дивергентных зонах (память: `(ssp_/ssp_)→(ssp_/ssp_)`), а ничто это не ловит.
...
const APPS_DIR = path.resolve(__dirname, '..', '..', '..'); // ~/internal
const allow = require('./fork-identical.json');

/* Определяем свой форк по namespace в core.js. */
const ownIsCorp = fs.readFileSync(path.join(OWN_SRC, 'core.js'), 'utf8').includes('__XFORK_');
/* sed В СТОРОНУ sibling: corp→comm как в allow.sed; comm→corp — реверс пар. */
const sed = ownIsCorp ? allow.sed : allow.sed.map(([a, b]) => [b, a]);
/* Каталог sibling-форка (другой проект под ~/internal/). */
const SIBLING_NAMES = ['Smart Sprint Planner', 'Internal Fork'];
let siblingSrc = null;
