...
test('шаг 2: editingFromHistory/historyIdx на WRITE молча стрипаются (прецедент gantt v6.1.0), без WARN, whitelist их не знает', () => {
  ...
  assert.strictEqual(stored.pluginVersion, '3.27.0');
...
test('шаг 2: миграция на READ — sprint 3.6.0 с legacy-ключами → ключей нет, SCHEMA_BUMP 3.6.0→3.23.0→3.27.0', () => {
  ...
  assert.strictEqual(s.pluginVersion, '3.27.0');
  assert.deepStrictEqual(s.migrationLog.filter((e) => e.level === 'SCHEMA_BUMP').map((e) => e.fromVersion + '→' + e.toVersion), ['3.6.0→3.23.0', '3.23.0→3.27.0']);
