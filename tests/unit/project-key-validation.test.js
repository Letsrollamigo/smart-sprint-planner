...
// Регресс: проект «1С ЗУП» (ключ 1c_zup) пропадал из global-picker'а, т.к. прежняя
...
describe('isValidProjectKey — допустимые ключи YouTrack (любой формат)', () => {
  const valid = [
    '1c_zup',        // ← регресс-кейс: «1С ЗУП», старт с цифры + нижний регистр + подчёркивание
    'DEMO', 'DEMOClone', 'MyProj', 'SCBT_2025',
...
describe('isValidProjectKey — точечно регресс «1С ЗУП»', () => {
  it('1c_zup проходит (раньше падал на ^[A-Za-z])', () => {
    assert.equal(isValidProjectKey('1c_zup'), true);
  });
  it('trim применяется', () => {
    assert.equal(isValidProjectKey('  1c_zup  '), true);
  });
