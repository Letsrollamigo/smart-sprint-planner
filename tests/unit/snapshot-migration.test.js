...
test('SCHEMA_MIGRATIONS: свёрнутая 1.4.2 → 3.6.0 + hard-removal 3.6.0 → 3.23.0 + no-op 3.23.0 → 3.27.0 → 3.28.0 → 3.29.0 → 3.32.0 → 3.35.0 → 3.39.0', function () {
  assert.ok(Array.isArray(SCHEMA_MIGRATIONS));
  assert.strictEqual(SCHEMA_MIGRATIONS.length, 8,
    'Registry: свёрнутая запись + 3.23.0 + 3.27.0 + 3.28.0 + 3.29.0 + 3.32.0 + 3.35.0 + 3.39.0; при следующем schema-change добавляй from="3.39.0"');
...
test('migrateSnap: legacy snapshot получает восемь SCHEMA_BUMP — 3.6.0, 3.23.0, 3.27.0, 3.28.0, 3.29.0, 3.32.0, 3.35.0, 3.39.0', function () {
  ...
  assert.deepStrictEqual(bumps, ['3.6.0', '3.23.0', '3.27.0', '3.28.0', '3.29.0', '3.32.0', '3.35.0', '3.39.0']);
  assert.ok(!('editingFromHistory' in snap) && !('historyIdx' in snap), 'legacy-ключи вычищены миграцией');
});

test('migrateSnap: snapshot на 3.23.0 — шесть SCHEMA_BUMP до 3.39.0; на 3.39.0 — ни одного', function () {
  const snap = { pluginVersion: '3.23.0' };
  migrateSnap(snap);
  assert.strictEqual(snap.pluginVersion, '3.39.0');
  assert.deepStrictEqual(snap.migrationLog.map(function (e) { return e.fromVersion + '→' + e.toVersion; }), ['3.23.0→3.27.0', '3.27.0→3.28.0', '3.28.0→3.29.0', '3.29.0→3.32.0', '3.32.0→3.35.0', '3.35.0→3.39.0']);
  const cur = { pluginVersion: '3.39.0' };
  migrateSnap(cur);
