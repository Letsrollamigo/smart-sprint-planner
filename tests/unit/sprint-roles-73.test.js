...
test('#73 миграция 3.23.0 → 3.27.0: no-op — roles не досеивается и не теряется', () => {
  ...
  assert.strictEqual(bare.pluginVersion, '3.29.0');   /* цепочка достраивается дальше — #74 добавил no-op 3.27.0→3.28.0, 68-8 — 3.28.0→3.29.0 */
  ...
  assert.strictEqual(h[0].pluginVersion, '3.29.0');
});
