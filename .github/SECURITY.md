...
Applies to version **3.49.1**. The model is server-authoritative: deny-by-default, whitelist validators, defense against Prototype Pollution, and an explicit role model.
...
>
> **v3.46.0 — #121 “Excluded issues with a reason”: no new endpoints, groups or permissions. Three additive role-item keys `excludeReason` / `excludedAt` / `excludedBy` live on an item only while `inclusionStatus === 'INC_EXCLUDED'`: on composition writes (`POST sprint-data` without `action` and `?action=validate`, the shared `roleItems` pre-flight) the server strips them for other statuses, refuses a reason longer than 500 characters after trim (`reason_too_long`), and on a transition into `INC_EXCLUDED` requires a non-empty reason (`reason_required`) and sets the who/when stamp itself (`fullName || login`, client values are ignored); the transition reference is the working slot when it holds the sprint, otherwise the history snapshots `<sprintId>_<rk>`; an already excluded item without a reason (data from before 3.46.0) passes as is. History writes (`POST history`, `?action=snapshot`, `import-replace`) check shape only (`validateItem`: string ≤ 1000, number). Schema marker `CURRENT_PLUGIN_VERSION` 3.45.0 → 3.46.0 (no-op migration), rollback floor 3.46.0 (`SYNC_PROTOCOL §E.5`).**
>
> **v3.45.0 — #120 “Work phases”: a new mode `POST sprint-data?action=phases` (satellite `backend-phases.js`, rights editor ∨ validator — no new groups or permissions) is the only write path for the keys `phases` / `phasesUpdatedAt` / `phasesUpdatedBy` in the working slot and history snapshots: the shape is validated strictly on write (keys ⊆ the six phases, a `dateStart`/`dateEnd` pair of numbers or `null`, end not before start by calendar day), on read a broken shape is dropped with a `WARN_PHASES_DROPPED` mark; the “who and when” stamps are server-side, client values are ignored; every other sprint/history write (`POST sprint-data`, `POST history`, `?action=snapshot`) overrides submitted phases with the stored ones (no carrier — accepted after shape validation; `import-replace` accepts deliberately, as a backup restore). `baseRev` is required as a number but is checked against the slot revision only when the slot holds this sprint; the history revision is bumped on fan-out. The `phasesEnabled` toggle and the `phaseRoles` mapping are admin-tier settings; a disabled toggle rejects writes with `phases_disabled`. Schema marker `3.40.0 → 3.45.0` (additive keys, no-op migration), rollback floor — 3.45.0. #127 “Estimate” without a confirmation dialog and #128 search-as-you-type are client-only, the rights model is unchanged.**
...
> **v3.39.1 — #116: no change to the model; calendar dates (sprint, tasks, date fields) are read and written in UTC (`pure/date-pure.js`); server and permissions untouched.**

> **v3.39.0 — #114: no change to the permission model. The new optional history-snapshot key `agreed` (the agreed composition baseline) passes a strict validator on write — only `at`/`by`/`items`, up to 1000 issues, ids up to 64 characters, per-issue only `e`/`x` — and is dropped on read with a `WARN_AGREED_DROPPED` migration-log entry. The slot revision probe on tab focus is the same `GET sprint-data` under the user's own permissions; no new endpoints or rights.**

> **v3.38.1 — #75: no change to the model; the TTM report reads the “Hierarchy” role from the links settings instead of a name regex (read-only, same permissions).**
>
...
> - **20** — the native language `<select>` in the header is recorded as an explicit exception to the Ring UI mandate (in the project's UI rules); no code change.
...
| 14 | **Conflict replay (overwriting concurrent edits)** *(restated in v3.17.0 — per code)* | The `baseSnapshotHash` comparison happens **on the client** (the “Version conflict” flow); the server only validates the field as a string. The server-side protection is the `baseRev` optimistic lock on the `sprint-data`/`history`/`releases`/`absences` slots: a mismatch → `rev_conflict`. Since 3.38.0 the lock is mandatory on `sprint-data`: a body with `sprint`/`roleItems` and no numeric `baseRev` is refused with `400 base_rev_required`. On `history`/`releases`/`absences` the lock stays advisory (a client without `baseRev` passes last-write-wins) — they have no external contract, and a mandatory lock would break the legacy raw `absences` body (#67 H11). |
