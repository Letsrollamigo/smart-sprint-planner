...
Актуально для версии **3.49.1**. Модель — server-authoritative: deny-by-default, whitelist-валидаторы, защита от Prototype Pollution и явная ролевая модель.
...
>
> **v3.46.0 — #121 «Исключённые задачи с причиной»: новых эндпоинтов, групп и полномочий нет. Три аддитивных ключа элемента роли `excludeReason` / `excludedAt` / `excludedBy` живут на элементе только при `inclusionStatus === 'INC_EXCLUDED'`: на записи состава (`POST sprint-data` без `action` и `?action=validate`, общий pre-flight `roleItems`) сервер срезает их при других статусах, отвергает причину длиннее 500 символов после trim (`reason_too_long`), а на переходе элемента в `INC_EXCLUDED` требует непустую причину (`reason_required`) и сам ставит отметки «кто/когда» (`fullName || login`, клиентские значения игнорируются); эталон перехода — рабочий слот, если он держит этот спринт, иначе снимки истории `<sprintId>_<rk>`; уже исключённая без причины (данные до 3.46.0) проходит как есть. Запись истории (`POST history`, `?action=snapshot`, `import-replace`) проверяет только форму (`validateItem`: строка ≤ 1000, число). Маркер схемы `CURRENT_PLUGIN_VERSION` 3.45.0 → 3.46.0 (миграция no-op), пол отката — 3.46.0 (регламент `SYNC_PROTOCOL §E.5`).**
>
> **v3.45.0 — #120 «Фазы работ»: новый режим `POST sprint-data?action=phases` (сателлит `backend-phases.js`, права editor ∨ validator — новых групп и полномочий нет) — единственный путь записи ключей `phases` / `phasesUpdatedAt` / `phasesUpdatedBy` в рабочем слоте и снимках истории: форма проверяется строго на записи (ключи ⊆ шести фаз, пара `dateStart`/`dateEnd` числами или `null`, конец не раньше начала по дням), на чтении битая форма снимается с пометкой `WARN_PHASES_DROPPED`; штампы «кто и когда» — серверные, клиентские значения игнорируются; все прочие записи спринта и истории (`POST sprint-data`, `POST history`, `?action=snapshot`) переопределяют присланные фазы хранимыми (носителя нет — принимаются после проверки формы; `import-replace` принимает осознанно, как восстановление бэкапа). `baseRev` обязателен числом, но сверяется с ревизией слота только если слот держит этот спринт; ревизия истории при fan-out бампается. Тумблер `phasesEnabled` и маппинг `phaseRoles` — admin-тир настроек; выключенный тумблер отвергает запись `phases_disabled`. Маркер схемы `3.40.0 → 3.45.0` (аддитивные ключи, миграция no-op), пол отката — 3.45.0. #127 «Оценка» без окна подтверждения и #128 поиск по вводу — только клиент, модель прав не менялась.**

> **v3.44.0 — #124: ссылки на ёмкость и релизы — только клиент, новых эндпоинтов, ключей хранения и прав нет. Ссылка несёт только `projectKey`, `sprintId`, `node` и `focus`; `node` берётся из фиксированного списка, `focus` обязан пройти `FOCUS_RE` (`^(role|user|hist|release):[A-Za-z0-9._@-]+$`) и при сборке, и при разборе ссылки, остальное отбрасывается. Ссылка только приводит на экран — что получатель увидит или сможет править, решают его собственные права; кнопки «Скопировать ссылку» появляются только в режиме главного меню при `host.navigation`.**
>
...
> **v3.39.1 — #116: модель не менялась; календарные даты (спринт, задачи, поля «дата») читаются и пишутся по UTC (`pure/date-pure.js`), сервер и права не тронуты.**

> **v3.39.0 — #114: модель прав не менялась. Новый необязательный ключ снимка истории `agreed` (слепок согласованного состава) проходит строгий валидатор на записи — только `at`/`by`/`items`, до 1000 задач, id до 64 символов, у задачи только `e`/`x` — и на чтении снимается с пометкой `WARN_AGREED_DROPPED` в журнале миграций. Зонд ревизии слота при возврате фокуса — тот же `GET sprint-data` под правами пользователя; новых эндпоинтов и полномочий нет.**

> **v3.38.1 — #75: модель не менялась; отчёт TTM читает роль «Иерархия» из настроек связей вместо регекспа по имени типа (только чтение, те же права).**
>
> **v3.38.0 — #110 волна 2: модель прав не менялась; запись `sprint`/`roleItems` через `POST sprint-data` без числового `baseRev` отвергается `base_rev_required` — раньше такое тело писалось last-write-wins, то есть риск №14 «conflict-replay» для REST-клиентов закрывался только текстом гайда, теперь закрыт сервером; 409 `rev_conflict` этого эндпоинта несёт `cid` и пишет строку лога, как остальные отказы.**
>
> **v3.37.0 — #85/#110: модель не менялась; в конверт отказов добавлен `cid`, на сервере при отказе пишется одна строка `cid + статус + код причины` без тела запроса и пользовательских значений (не гейтится enableDebugLog — иначе cid не по чему искать).** Повтор чтений касается только GET и проходит те же authz-гейты на каждой попытке.
>
> **v3.36.1 — #111: модель не менялась; исчез один запрос `config?fields=version` из главного меню** (адрес вкладки больше не зависит от версии инстанса).
>
> **v3.36.0 — #89/#90/#109: модель безопасности не менялась.** Счётчик задач без оценки и предпросмотр импорта истории — чистые вычисления на данных, уже загруженных под правами пользователя; предпросмотр считает тем же планом, что и запись, новых эндпоинтов и ключей схемы нет. Полоса «настройки не подхвачены» в главном меню только читает уже доступное зеркало настроек и строит ссылку на вкладку проекта.
>
> **v3.35.0 — #88 «Ролевое поле спринта»: новых эндпоинтов нет, появился новый ПУТЬ записи в задачи.** В `SETTINGS_WHITELIST` добавлены девять ролевых ключей `fieldSprint<Роль>` (планировочный тир, как остальные `field*`) и булев `sprintWriteEnabled`; в `ALLOWED_SPRINT_KEYS` — необязательная карта `sprintFieldValByRole` (ключи только ролевого вида, значения — строки ≤500, ≤50 записей). Миграция 3.32.0 → 3.35.0 no-op, регресс — фикстуры версии-границы. **Своего эндпоинта фича не заводит:** запись идёт уже существующим `POST update-issue-field`, чей allow-list строится фильтром по `field*`/`userField*`-ключам хранимых настроек и новые ключи подхватывает сам — то есть писать по-прежнему можно ТОЛЬКО в поля, назначенные настройками плагина. Все замки этого эндпоинта остаются на месте: роль `assigner`, изоляция проекта, `Issue.isVisibleTo` и `Issue.canBeWrittenBy` под правами самого пользователя (#67 Q1). Новое в модели рисков — сам факт массовой записи: канал выключен по умолчанию, срабатывает только на согласовании состава роли (не на каждое добавление задачи), идёт чанками по 25 и **никогда не очищает поле** — снятие значения уничтожало бы данные, внесённые вне планера. Многозначные поля отбиваются до записи: присваивание заменило бы весь список значений задачи. Authz-гейты, группы и матрица доступа — без изменений.
...
> - **20** — нативный `<select>` языка в шапке зафиксирован явным исключением из мандата Ring UI (в правилах проекта по UI), кода не касается.
...
| 14 | **Conflict-replay (перезапись чужих параллельных правок)** *(переформулировано в v3.17.0 — по коду)* | Сверка `baseSnapshotHash` выполняется **на клиенте** (конфликт-флоу «Конфликт версий»); сервер поле только валидирует как строку. Серверная защита — optimistic lock `baseRev` на слотах `sprint-data`/`history`/`releases`/`absences`: расхождение → `rev_conflict`. С 3.38.0 на `sprint-data` lock mandatory: тело с `sprint`/`roleItems` без числового `baseRev` отвергается `400 base_rev_required`. На `history`/`releases`/`absences` lock остаётся advisory (клиент без `baseRev` проходит last-write-wins) — внешнего контракта у них нет, mandatory сломал бы legacy raw-`absences` (#67 H11). |
