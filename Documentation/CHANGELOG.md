...
- **Cross-fork compatibility.** Files exported from the corporate fork (`scbt-sprint-history`) import here with an info note; records are always written to `ssp_history` regardless of the file's origin marker.
...
> **Parity gate with corp full-rebuild lineage.** 8 UX/CSS fixes consolidated from corp v2.1.1→v2.1.7 (Olear corp full-rebuild iteration). Modal click-anchor jump root cause + Ring Table CSS-module silent-noop selectors + defensive multiline textarea sizing + Ring font-size 13px scoped override. No schema changes. 424 unit tests pass.
...
- **Parity gate context:** corp `Sprint Planer for MultiTeams` (private fork) completed a full-rebuild on community v2.1.0 codebase (2026-05-25) and accumulated 8 hotfixes during smoke iteration v2.1.1→v2.1.7. This release back-ports the 8 common UX/CSS fixes from corp lineage so community matches the same UX baseline. Corp-specific commits (namespace renames, `BASELINE_ASSUMED` bridge, `scbt-state-toolkit`, `settings.json` `type:string` choice, `SSP_VENDORED→SCBT_VENDORED` cleanup, hardcoded `/app-version '2.1.0'` corp bug) are NOT back-ported — see `Youtrack APPS/Shared Docks/DIFF_MAP.md` and `PARALLEL_DEV_STRATEGY.md §7`.
