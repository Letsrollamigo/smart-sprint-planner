...
> 🎉 **Smart Sprint Planner в [JetBrains Marketplace](https://plugins.jetbrains.com/search?search=smart%20sprint%20planner).** Версия **2.14.0** — последний одобренный релиз. Marketplace-листинг — канонический способ установки для команд, которым нужны проверенные стабильные релизы.
...
- **`ssp-main-global` (MAIN_MENU_ITEM)** — полный планер в главном меню YouTrack: дашборд «рельс + пейн» (на широком экране — панель навигации слева ~210px + рабочая область; на узком — стопка), выбор проекта в шапке-рельсе, дерево навигации (Параметры спринта / Планирование / Гант / История). Здесь идёт всё планирование. Планирование — два уровня: «Аллокация общего ресурса» (accordion-карточки ролей с editable-составом и вводом ёмкости — вручную либо расчётом) и «Распределение по исполнителям» (per-role селектор, ассайни и даты). Второй уровень виден **только при включённом персональном планировании**; без него (режим «super-light») распределение исполнителей ведётся на диаграмме Ганта.
...
|---|---|---|---|
| **[JetBrains Marketplace](https://plugins.jetbrains.com/search?search=smart%20sprint%20planner)** | **v2.14.0** | Стабильные релизы, проверены JB-ревью | Командам, которым нужны проверенные релизы и встроенный auto-update YouTrack. Каждая новая выгрузка проходит модерацию JetBrains marketplace (1–3 рабочих дня) перед публикацией. |
...
- **Распределение задач по исполнителям** с колонкой «Система» (read-only, сортируемая) и опциональной колонкой «Аллокации по проектам» — per-system breakdown часов и процентов от capacity исполнителя. Если режим персонального планирования выключен (режим «super-light»), пункт навигации «Распределение по исполнителям» скрыт: ёмкость роли вводится вручную в «Аллокации общего ресурса», а ответственных за задачи назначают прямо на диаграмме Ганта; поучастный учёт ёмкости не ведётся.
- **Раздел настроек «Управление ёмкостью»** — параметры расчёта ёмкости (нормы / ставка / процент участия / коэффициенты грейдов) и флаги источника ресурса (режим персонального планирования, авторасчёт общего ресурса из персонального планирования, ручной ввод ресурса по исполнителям) собраны в админ-раздел, который правит только настройщик проекта.
