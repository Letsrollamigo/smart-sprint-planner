...
- **Проект не виден в выборе проектов (пикер).** Проект, чей ключ (shortName) начинается с цифры или содержит кириллицу/дефис — например «1С ЗУП» (`1c_demo`) — молча отсутствовал в выпадающем списке проектов планера. Причина: фильтр picker'а проверял ключ строгой ASCII-регуляркой (`^[A-Za-z][A-Za-z0-9_]…`) и отбрасывал всё остальное. Валидация ключа заменена на проверку длины + denylist опасных символов — пропускаются любые реальные ключи YouTrack (цифра в начале, кириллица, дефис, точка, подчёркивание). Та же регулярка чинит и открытие deep-link'ов (#36) на такие проекты.
